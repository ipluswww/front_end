...
.directive('txsplain', ['$http', 'PortalName', function($http, PortalName) {
  ...
  var txFlags = {
    ...
    TrustSet: {
      ...
      0x00020000: 'tfSetNoPortal',
      0x00040000: 'tfClearNoPortal',
  ...
  return {
    ...
    link: function(scope, element) {
      ...
      function renderMeta(tx) {
        ...
        // render PortalSate
        function renderPortalState(action, node) {
            ...
            nodes.forEach(function(node) {
              switch (node.LedgerEntryType) {
                ...
                case 'Offer':
                  ...
                  break;
                case 'PortalState':
                  html += renderPortalState(type, node);
                  break;
                default:
                  html += '<li>It ' + type +
      ...
      function renderDescription(tx) {
        ...
        function renderOfferCreate() {
          ...
          if (tx.tx.Expiration) {
            var expiration = moment.unix(tx.tx.Expiration + Portal_EPOCH);
      ...
      function displayTx() {
        ...
        div.selectAll('account').each(function() {
          ...
          PortalName(account, function(name) {
            if (name) {
              el.html('<t>~</t><name>' +
