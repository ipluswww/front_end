angular.module('chordDiagram', []).directive('chordDiagram', ['$window', 'matrixFactory', 'PortalName',

function ($window, matrixFactory, PortalName) {
...
*/
    ...
    function updateSidebars(d) {
      ...
      div.selectAll('.issuer:not(.checked)').each(function(d) {
        ...
        if (issuer) {
          PortalName(issuer, function(name) {
            el.text(name || issuer)
