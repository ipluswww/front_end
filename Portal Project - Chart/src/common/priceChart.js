...
if (!LOADER_PNG) {
  var LOADER_PNG = 'assets/images/PortalThrobber.png'
}
...
function PriceChart(options) {
  ...
  // format an amount if Portal-lib is present
  function PortalAmount(amount, currency) {
    if (typeof Portal === 'undefined' || !Portal.Amount) {
      return amount
    }

    return Portal.Amount.from_human(amount + ' ' + currency)
  ...
  function parseDate(date, increment) {
    switch (increment) {
      case 'da':
        return date.utc().format('MMMM D') +
          ' <small>(' + date.utc().format('hh:mm A') +
          ' UTC)</small>'
      case 'mo':
        return date.utc().format('MMMM YYYY') + ' <small>UTC</small>'
      default:
        return date.format('MMMM D') +
          ' &middot ' + date.format('hh:mm:ss A') + ' ' + tz(date)
    }
  }
  ...
  wrap = options.id ?
    d3.select('#' + options.id) : d3.select('body').append('div')
  ...
  function drawChart() {
    ...
    focus = gEnter.append('circle')
      .attr('class', 'focus dark')
      .attr('r', 3)
  ...
  function liveUpdate(data) {
    ...
    if (last && last.startTime.unix() === candle.startTime.unix()) {
      ...
      if (first && first.startTime.unix() < startTime.unix()) {
        lineData.shift()
      }
    }
  ...
  function getAlignedCandle(time) {
    ...
    if (chartInterval === 'se') {
      ...
      if (multiple === 1) {
        ...
        if (diff < 0) {
          days = multiple - (0 - Math.floor(diff)) % multiple
        } else {
          days = Math.floor(diff) % multiple
        }
      }
  ...
  // enable the live feed via Portal-lib
  ...
  this.load = function(b, c, d) {
    ...
    if (!c) {
      setStatus('Counter currency is required.')
      return
    }

    if (!d || !d.interval) {
      setStatus('Interval is required.')
      return
    }
    ...
    switch (chartInterval) {
      ...
      case ('we'):
        ...
        break
      case ('mo'):
        intervalSeconds = 60 * 60 * 24 * 30.5 // approx
        break
      default:
        setStatus('Invalid Interval')
    }
    ...
    lastCandle = getAlignedCandle(moment.utc(d.end))
    endTime = moment.utc(lastCandle).add(intervalSeconds, 'seconds')
    startTime = d.start ?
      getAlignedCandle(moment.utc(d.start)) : moment.utc(d.offset(endTime))
    ...
    self.request = apiHandler.offersExercised({
      ...
      if (lineData.length && data.length) {
        ...
        if (candle.high > first.high) {
          candle.high = first.high
        }

        if (candle.low < first.low) {
