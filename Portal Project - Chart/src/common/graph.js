networkGraph = function (nameService) {
  ...
  // CONSTANTS
  var UNIX_Portal_TIME = 946684800;
  ...
  function handleTransaction(obj) {
    ...
    obj.date = moment((UNIX_Portal_TIME + obj.tx.date)*1000).format();
  ...
  function eraseGraph() {
    ...
    $(".loading").html('<img class="loader" src="assets/images/PortalThrobber.png" style="vertical-align: middle;" /> Loading...');
  }
...
function updateTransactions(address) {
  ...
  if (nodes[nodeMap[address]].transactions) {
          ...
          for (var j=0; j<meta.AffectedNodes.length; j++) {
            ...
            if (mn && LatestFields) {
              if (LatestFields.Account == address || (LatestFields.HighLimit && LatestFields.HighLimit.issuer == address) ) {
                ...
                } else if (type == "PortalState") {
                  //console.log("Affected PortalState:", mn);
    ...
    if (!nodes[nodeMap[address]].transactionsFinished) { //Are there more?
      $('#transactionTable').append('<tr id="transactionThrobber"><td colspan=3 style="text-align:center; padding:10px"><img class="loader" src="assets/images/PortalThrobber.png" width=30 height=30 /></td></tr>');
  ...
  function init () {
      ...
      if (firstTime) {
        if (transaction_id && transaction_id !== "") {
          ...
          $('#focus').val(PortalName);
          nameService(PortalName, function(name, address){
            if (address) {
              focalNode = address;
