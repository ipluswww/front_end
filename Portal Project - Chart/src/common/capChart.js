...
function CapChart(options) {
  ...
  var legendDiv = div.append('div').attr('class', 'legend');
  ...
  function getTooltipPosition(cx, cy) {
    ...
    if (cx + 120 > options.width + options.margin.right) {
      tx = cx - 260;
    } else if (cx - 120 < options.margin.left) {
      tx = cx + 20;
    } else {
      tx = cx - 120;
    }

    if (cy - 120 < options.margin.top) {
      ty = cy + 120;
    } else {
      ty = cy - 120;
    }
  ...
  function handleTracer(cx, cy, c2y) {
    ...
    tracer.select('.top').transition().duration(dur).attr({
      cx: cx,
      cy: cy
    });

    if (c2y) {
      tracer.select('.vertical')
        .transition()
        .duration(dur)
        .attr({
          x1: cx,
      ...
      tracer.select('.horizontal')
        .transition()
        .duration(dur)
        .style('opacity', 0);

      tracer.select('.bottom')
        .transition()
        .duration(dur)
        .attr({
          cx: cx,
      ...
      tracer.select('.vertical')
        ...
        .attr({
          ...
          y2: options.height + options.margin.top
        });
  ...
  function handleTooltip(title, address, date, amount, position) {
    ...
    tooltip.append('h5')
      .html(title)
      .style('color', address ? color(address) : 'inherit');
  ...
  function movingInSky() {
    ...
    if (self.format === 'stacked') {
      ...
      if (i && date < (top[i].date + top[i - 1].date) / 2) {
        i--;
      }
  ...
  function movingInGround(section) {
    ...
    if (date < (section.values[i].date + section.values[i - 1].date) / 2) {
      i--;
    }

    var cy = yScale(section.values[i].y +
      section.values[i].y0) +
      options.margin.top;
    ...
    var amount = commas(section.values[i].y);
  ...
  function movingOnLine(line) {
    ...
    var position = getTooltipPosition(cx, cy);
    var name = line.name;
    var amount = commas(line.values[i].y);
  ...
  function drawChart() {
    ...
    if (!svg) {
      ...
      loader = chart.append('img')
        .attr('class', 'loader')
        .attr('src', 'assets/images/PortalThrobber.png');
    }
    ...
    legend = raw.map(function(d) {
      ...
      if (d.values) {
        amount = d.values.length ?
          commas(d.values[d.values.length - 1].y) : 0;
      } else {
        amount = d.results.length ?
          commas(d.results[d.results.length - 1][1]) : 0;
      }
  ...
  function loadTradeHelper(range, base, count) {
    ...
    var currency = self.currency;
    var end = moment.utc();
    var int = range.interval === 'week' ? 'day' : range.interval;
  ...
  // load capitalization data helper
  ...
  if (!options.width) {
    options.width = parseInt(chart.style('width'), 10) -
