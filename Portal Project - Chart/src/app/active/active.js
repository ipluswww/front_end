angular.module( 'portalobject.activeAccounts', [
  'ui.state',
  'ui.bootstrap'
])
...
//set up the currency pair dropdowns
  ...
  $scope.$watch('theme', function(){
    dropdownB = Portal.currencyDropdown(gateways).selected(counter)
      .on("change", function(d) {
        counter = d;
        loadPair();
      });
    dropdownA = Portal.currencyDropdown(gateways).selected(base)
      .on("change", function(d) {
        base = d;
        loadPair();
      });
