angular.module( 'portalobject.manage-gateways', [
  'ui.state',
  'ui.bootstrap'
])
...
.controller( 'ManageGatewaysCtrl', function ManageGatewaysCtrl( $scope, $state, $location, gateways ) {
  ...
  function add(){
    ...
    d3.xhr('https://id.Portal.com/v1/user/'+newGateway, function(err, res){
      if (!err) {
      ...
      if (newGateway.length >= 30 && newGateway.length <= 40 && newGateway[0] === "r"){
        ...
        description.html('Please enter a valid Portal Address.');
      }
    });
  }
