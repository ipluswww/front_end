...
/* global Portal, OffersExercisedListener, ApiHandler */
'use strict';
...
var remote = new Portal.PortalAPI({
  server: 'wss://s1.Portal.com:443'
});
remote.connect()
.then(function() {
  console.log('connected to the Portal network');
})
.catch(function(e) {
  console.log(e.stack);
});
  ...
  self.reqUtcPrice = self.markets.apiHandler.offersExercised({
    ...
    self.reqCurrentPrice = self.markets.apiHandler.offersExercised({
      ...
      self.div.on('click', function() {
        ...
        window.open('https://www.charts.Portal.com/#/' + path, '_blank');
      });
