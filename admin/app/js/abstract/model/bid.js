export default class Bid {
    constructor(order, isSpacePrice) {
        ...
        if(isSpacePrice){
            this.containers.push({ subType: order.storageUnit.subType._id });
        }
        else {
            if(order.goingToWarehouse){
                order.inventory.forEach((container)=>{
                    this.containers.push({ subType: container.subType._id });
                });
            }
            else {
                order.inventory.forEach((container)=>{
                    this.removeContainers.push({ _id: container._id });
                });
            }
        }
    }
