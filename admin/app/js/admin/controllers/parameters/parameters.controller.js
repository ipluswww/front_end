...
class  ParametersController {
	...
	constructor ($q, User, Parameter, QueryData, $rootScope, $state, $window){
		this._$q = $q;
		this._User = User;
	...
	init () {
		...
		this.query = this._QueryData.getQuery('parameters') || this.query;
		...
		this._$rootScope.$on('$stateChangeStart',
			function(event, toState, toParams, fromState, fromParams){
				if (fromState.name === 'app.admin.parameter.list') self._QueryData.setQuery('parameters', self.query);
			});
	}
...
ParametersController.$inject = ['$q', 'User', 'Parameter', 'QueryData', '$rootScope', '$state', '$window'];
