...
class  MarketsController {
	...
	constructor ($q, User, Market, QueryData, $rootScope, $state, $window){
		this._$q = $q;
		this._User = User;
	...
	init () {
		...
		this.query = this._QueryData.getQuery('markets') || this.query;
		...
		this._$rootScope.$on('$stateChangeStart',
			function(event, toState, toParams, fromState, fromParams){
				if (fromState.name === 'app.admin.market.list') self._QueryData.setQuery('markets', self.query);
			});
	}
...
MarketsController.$inject = ['$q', 'User', 'Market', 'QueryData', '$rootScope', '$state', '$window'];
