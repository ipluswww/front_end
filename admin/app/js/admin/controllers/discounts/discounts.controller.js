...
class  DiscountsController {
	...
	constructor ($q, User, Discount, QueryData, $rootScope, $state, $window){
		this._$q = $q;
		this._User = User;
	...
	init () {
		...
		this.query = this._QueryData.getQuery('discounts') || this.query;
		...
		this._$rootScope.$on('$stateChangeStart',
			function(event, toState, toParams, fromState, fromParams){
				if (fromState.name === 'app.admin.discount.list') self._QueryData.setQuery('discounts', self.query);
			});
	}
...
DiscountsController.$inject = ['$q', 'User', 'Discount', 'QueryData', '$rootScope', '$state', '$window'];
